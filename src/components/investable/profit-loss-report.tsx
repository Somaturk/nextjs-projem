...
export default function ProfitLossReport({ portfolio, livePrices }: ProfitLossReportProps) {
  ...
  useEffect(() => {
    ...
    const calculatePnl = (asset: PortfolioAsset): ReportAsset | null => {
        ...
        if (asset.type === 'deposit' && asset.depositType === 'time') {
            ...
            initialValue = asset.purchasePrice * asset.amount;
            currentValue = currentPrice * asset.amount;
        } else {
            return null;
        }
    ...
    const reportableAssets = portfolio.map(calculatePnl).filter((a): a is ReportAsset => a !== null);
    
    let filteredAssets;
    if (timeRange === 'all') {
        filteredAssets = reportableAssets;
    } else {
        filteredAssets = reportableAssets.filter(asset => {
    ...
    filteredAssets.forEach((asset) => {
        ...
        groups[asset.type].totalInitialValue += asset.initialValue;
        groups[asset.type].totalCurrentValue += asset.currentValue;
        groups[asset.type].totalProfitLossAmount += asset.profitLossAmount;
    });

    const finalGroups = Object.keys(groups).map(type => {
        ...
        return { type: type as AssetType, ...group };
    }).sort((a, b) => b.totalCurrentValue - a.totalCurrentValue);

    setGroupedReportData(finalGroups);
