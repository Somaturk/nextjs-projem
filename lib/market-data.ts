...
const coingeckoCryptoMap: Record<string, { symbol: string, name: string }> = {
...
const getCoingeckoCryptoAssets = async (): Promise<Asset[]> => {
    const ids = Object.keys(coingeckoCryptoMap).join(',');
    const url = `https://api.coingecko.com/api/v3/simple/price?ids=${ids}&vs_currencies=try`;

    try {
        const response = await fetch(url);
        if (!response.ok) {
            console.warn(`CoinGecko API request failed with status: ${response.status}`);
            return [];
        }
        const data = await response.json();
        
        const assets: Asset[] = Object.entries(data).map(([id, priceData]) => {
            const price = (priceData as any).try ?? 0;
            const info = coingeckoCryptoMap[id];
        ...
        console.error(`Error fetching or parsing data from CoinGecko:`, error);
        return [];
    }
}
...
export const getAvailableAssets = async (type: AssetType, usdToTryRate: number = 1): Promise<Asset[]> => {
  if (type === 'crypto') {
      return getCoingeckoCryptoAssets();
  }
  
  const endpointMap: Record<string, string> = {
    currency: 'allCurrency',
    gold: 'goldPrice',
    // crypto is handled above
    stock: 'hisseSenedi',
    silver: 'silverPrice',
  };
  ...
  try {
    ...
    if (data && data.success && data.result) {
        ...
        if (transformedData.length === 0 && type !== 'stock') {
            console.warn(`API for ${type} returned empty or invalid data after transformation.`);
            return [];
        }
        return transformedData;
    }
    
    console.warn(`API for ${type} call was not successful or result was empty.`);
    return [];
  } catch (error) {
